...
impl AppState {
    fn update_ui(&self, display: &mut SimulatorDisplay<BinaryColor>) -> Result<(), Infallible> {
...
fn main() -> Result<(), core::convert::Infallible> {
    let state = AppState {
        freq_khz: 23_400,
        volume: 15, // 0 to 15
        station_info: "XXXXXXXXXXXXXXXXXXXYYYYYYYYYYYYYYYYYYY".to_string(),
        rssi: 40,
        cursor_at: UIElement::FreqControl,
        element_is_active: true,
    };

    let mut display = SimulatorDisplay::<BinaryColor>::new(Size::new(128, 64));

    state.update_ui(&mut display)?;

    let output_settings = OutputSettingsBuilder::new()
        .theme(BinaryColorTheme::OledBlue)
        .build();
    Window::new("Radio Display", &output_settings).show_static(&display);
